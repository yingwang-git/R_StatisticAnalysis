...
## merge data
```{r}
data_merge <- merge(data1, data2, by.x = "ID", by.y = "ID", all.x = TRUE)
```
...
# 求行均值
data$row_mean <- rowMeans(data[, 2:5])
```
...
Hmisc::describe(data)  # 另一种更详细的描述性
```
...
windowsFonts(Times=windowsFont("Times New Roman"))  # 更改字体
...
# 好看的折线图（因变量连续，自变量分类）
ggplot(data, aes(x=factor(LEVEL, level = c("level1", "level2", "level3")),
                 y=y, group=1)) + 
  geom_path(size = 1) + 
  geom_point(size = 2) + 
  xlab("x") + 
